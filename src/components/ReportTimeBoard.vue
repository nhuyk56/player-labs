...
import { generateReportTimeBoard, downloadExcelReport } from "./helpers";
...
const registerEventAudio = () => {
  ...
  const handlerEnded = () => {
    if (player.value.src !== srcMp3.value) {
      ...
      player.value.playbackRate = formData.value.playbackRate;
      player.value.play();
    } else {
      const currentChapterIndex = chaptersOptions.value.findIndex(
        (ch) => ch.id === formData.value.chapter_id
      );
...
const handlerChapterChange = async () => {
  const chapterFound = chaptersOptions.value.find(
    (ch) => ch.id === formData.value.chapter_id
  );
  handlerChapterPlay(chapterFound);
};
...
const handlerPlaybackRate = (playbackRate) => {
  formData.value.playbackRate = playbackRate;
  player.value.playbackRate = playbackRate;
};
  ...
  <div class="container flex flex-col pb-5">
    ...
    <div class="flex-initial w-full mx-auto mt-2">
      <n-select
        ...
        @update:value="handlerChapterChange"
    ...
    <div class="flex-initial w-full mx-auto">
      ...
      <div class="flex justify-center text-xs" :disabled="!srcMp3">
        ...
        <n-slider
          v-model:value="formData.playbackRate"
          :step="0.1"
          :min="0"
          :max="5"
          :disabled="!srcMp3"
          class="w-1/5"
          @update:value="handlerPlaybackRate"
        />
      </div>
    </div>
